<?php 
    ...
    $query = "SELECT * FROM table_user WHERE networkid LIKE '$snetid%' AND LENGTH(networkid) = '$length' AND networkid != '1' AND level='4' AND status=1 ORDER BY LENGTH('networkid') ";

        $mySql_query = $db->select($query);
            if($mySql_query){
?>
            <h4>User Table # Level 4</h4>
